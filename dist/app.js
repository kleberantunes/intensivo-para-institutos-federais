...
function save(){
  localStorage.setItem(activeUserId ? `${STORAGE_KEY}:user:${activeUserId}` : STORAGE_KEY, JSON.stringify(progress));
...
window.useIFAccount = (uid) => {
  ...
  Object.assign(progress, emptyProgress(), JSON.parse(accountData || guestData || '{}'));
  activeUserId=uid;
  if(guestData)localStorage.removeItem(STORAGE_KEY);
};
...
document.addEventListener('click',e=>{
  ...
  if(!b)return;
  if(b.dataset.route==='home' && currentRouteName==='login')sessionStorage.setItem('ifintenso-guest','1');
  route(b.dataset.route,b.dataset.arg);
});
$('#menuBtn').onclick=()=>$('#nav').classList.toggle('open');
window.onIFLogin = () => {sessionStorage.removeItem('ifintenso-guest'); route('home');};
window.onIFLogout = () => {
  if(activeUserId)save();
  activeUserId=null;
  for(const key of Object.keys(progress))delete progress[key];
  Object.assign(progress, emptyProgress(), JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}'));
  sessionStorage.removeItem('ifintenso-guest');
  route('login');
};

function login(){
  if(window.cloudSync?.isReady()){
    app.innerHTML=`<div class="shell narrow"><section class="login-card"><span class="eyebrow">Sua conta</span><h1>Progresso sincronizado.</h1><p>Você está conectado com o Google. Seus resultados são salvos na nuvem quando a sincronização está disponível.</p><div class="actions"><button class="btn primary" data-route="home">Continuar estudando</button><button class="btn light" data-route="progress">Ver progresso</button></div></section></div>`;
    return;
  }
  app.innerHTML=`<div class="shell narrow"><section class="login-card"><span class="eyebrow">IFintenso · Seu estudo começa aqui</span><h1>Prepare-se para os Institutos Federais.</h1><p>Pratique com questões de provas anteriores de todo o Brasil. Entre com o Google para acompanhar seu progresso em outros dispositivos.</p><div class="actions"><button class="btn primary" id="loginPageBtn" type="button">Entrar com Google</button><button class="btn light" data-route="home" type="button">Continuar sem conta</button></div><small>Sem conta, seu progresso fica apenas neste navegador.</small></section></div>`;
  $('#loginPageBtn').onclick=()=>{if(window.cloudSync)$('#loginBtn').click();else toast('Preparando o login. Tente novamente em instantes.');};
}
...
function home(){
  ...
  app.innerHTML=`<div class="shell">
    <section class="welcome">
      <div><span class="eyebrow">Sua preparação nacional</span><h1>Estude com questões de todo o Brasil.</h1><p>Questões por área, provas oficiais por estado e um plano que aprende com seus resultados.</p><div class="actions"><button class="btn primary" data-route="quiz">Começar simulado</button><button class="btn light" data-route="exams">Ver provas anteriores</button></div></div>
      <aside class="profile-card"><span class="profile-label">Meu progresso</span><strong>${progress.answered} questões respondidas</strong><span>${window.cloudSync?.isReady()?'Sincronização com Google ativada':'Salvo neste navegador'}</span><button data-route="progress">Ver desempenho</button></aside>
    </section>
    ...
    <section class="cards"><button class="feature" data-route="study"><span class="feature-number">01</span><h3>Estudar por matéria</h3><p>Escolha a área e pratique com questões de todo o Brasil.</p></button><button class="feature featured" data-route="quiz"><span class="feature-number">02</span><h3>Simulado nacional</h3><p>Uma rodada com questões de todo o país.</p></button><button class="feature" data-route="exams"><span class="feature-number">03</span><h3>Provas reais</h3><p>Encontre cadernos e gabaritos por estado.</p></button></section>
  </div>`;
}
...
function exams(){
  ...
  const filtered=progress.state?available.filter(s=>s.uf===progress.state):available;
  const options=data.states.map(s=>`<option value="${s[0]}" ${s[0]===progress.state?'selected':''}>${s[1]} (${s[0]})</option>`).join('');
  app.innerHTML=`<div class="shell"><div class="section-title"><div><span class="eyebrow">Acervo oficial</span><h1>Provas e gabaritos</h1><p>Escolha um estado para consultar as fontes oficiais disponíveis. Isso não limita o banco de questões.</p></div></div><label class="exam-filter">Estado <select id="examState"><option value="">Todos os estados</option>${options}</select></label>${filtered.length?`<section class="exam-list">${filtered.map(s=>`<a class="exam" href="${s.url}" target="_blank" rel="noopener"><div><span>${s.institution} · ${s.uf}</span><h2>${s.title}</h2><p>${s.detail}</p></div><strong>Ver acervo ↗</strong></a>`).join('')}</section>`:`<section class="empty"><h2>Ainda não há links cadastrados para este estado.</h2><p>Continue praticando com o banco nacional enquanto ampliamos o acervo.</p><button class="btn primary" data-route="study">Estudar agora</button></section>`}</div>`;
  $('#examState').onchange=e=>{progress.state=e.target.value;save();exams();};
}
...
if(sessionStorage.getItem('ifintenso-guest'))home(); else login();
